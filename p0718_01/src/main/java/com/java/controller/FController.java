...
//		emailService.email_send(name, email);
	...
	public ArrayList<LoanDto> chart_data(String data) {
		...
		//Loan 리스트: db에서 region,amt를 객체로 생성해서 출력하시오.
		if(data.equals("region")) list = loanService.selectLoan();
		else list = loanService.selectLoan2();
		for(var i=0;i<list.size();i++) {
			System.out.println(list.get(i).getRegion()+","+list.get(i).getAmt());
			System.out.println(list.get(i).getPeriod()+","+list.get(i).getAmt());
		}
		return list;
	}
}
